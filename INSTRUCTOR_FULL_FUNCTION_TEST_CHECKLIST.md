...
- **[FIX 01/09/2026 - IMPORT & ATTACHMENT SYNC]** Tạo mới [SelectQuizModal.tsx](file:///h:/du_an/website/mindnova-ai/src/features/instructor/quiz-generator/components/SelectQuizModal.tsx) và tích hợp nút **"📥 Chọn từ Ngân hàng Quiz"** vào `QuizEditor.tsx`: Cho phép Giảng viên mở danh sách đề thi đã tạo sẵn từ AI Quiz Generator để chèn/import vào bài học bất kỳ.
- **[FIX 01/09/2026 - BACKEND LINKING]** Sửa [LessonService.php](file:///h:/du_an/website/website-MindNova-AI/app/Services/Instructor/LessonService.php) & [QuizService.php](file:///h:/du_an/website/website-MindNova-AI/app/Services/Instructor/QuizService.php): Khi lưu bài học có gán `quiz_id` (từ bài thi chọn trong ngân hàng), Backend tự động gán `$quiz->lesson_id = $lesson->id` và tạo/cập nhật `QuizCourseAttachment`. Eager loading `'lesson.course'` trong `getInstructorQuizzes` giúp trang Quiz Generator (`/instructor/quiz-generator`) tự động chuyển badge từ `"📌 Chưa gắn"` thành `"🎓 Khóa: [Tên khóa học]"`.
- **[FIX 01/09/2026 - COURSE OUTLINE QUIZ AUTHOR]** Sửa [CourseOutlineController.php](file:///h:/du_an/website/website-MindNova-AI/app/Http/Controllers/Api/Instructor/CourseOutlineController.php): Gán `instructor_id` và tạo `QuizCourseAttachment` khi tạo bài thi từ dàn ý AI.
---
