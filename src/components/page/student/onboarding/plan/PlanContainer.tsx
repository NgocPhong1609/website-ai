  ...
  const phases = useMemo(
    (): IPlanPhase[] =>
      DEFAULT_PLAN_PHASES.map((phase, idx) => {
        ...
        const itemStatus =
          idx === 0 ? ("ready" as const) : ("upcoming" as const);
        return {
          ...phase,
...
function CelebrationBanner({ goal }: { goal: string }) {
  return (
    <div className="relative w-full max-w-4xl bg-gradient-to-r from-[#6B6BFF]/8 via-[#818cf8]/6 to-[#4cd7f6]/8 border border-[#6B6BFF]/15 rounded-2xl px-6 py-4 overflow-hidden">
      {/* Decorative shimmer */}
      <div
        className="absolute inset-0 bg-[radial-gradient(ellipse_at_0%_50%,rgba(107,107,255,0.12)_0%,transparent_60%)]"
        aria-hidden="true"
      />
      <div
        className="absolute inset-0 bg-[radial-gradient(ellipse_at_100%_50%,rgba(76,215,246,0.10)_0%,transparent_60%)]"
        aria-hidden="true"
      />
      ...
      <div className="relative flex items-center gap-4">
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <span className="text-xs text-[#64647A]">
            Crafted by AI based on your goal{goal ? ` — ${goal}` : ""}, skill
            level & selected topics.
          </span>
        </div>
      </div>
    </div>
    ...
    <div className="w-full flex flex-col items-center gap-8 px-6 py-12">
      ...
      <div className="flex flex-col items-center gap-3 text-center max-w-2xl">
        ...
        <p className="text-base text-[#64647A] leading-relaxed max-w-lg">
          Every phase is tailored to your unique goal and expertise — start when
          you&apos;re ready.
        </p>
      </div>
