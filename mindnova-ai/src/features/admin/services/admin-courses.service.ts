...
export async function getAdminCoursesPageData(params?: {
 ...
 const suffix = query.toString() ? `?${query.toString()}` : "";
  ...
  try {
    ...
    console.warn("[AdminCoursesService] Failed to fetch admin courses data:", error);
    return {
      summary: { published: 0, draft: 0, pendingReview: 0 },
      filters: { search: params?.search ?? "", categoryId: params?.categoryId ?? "", level: params?.level ?? "", categories: [] },
      rows: [],
    };
  }
}
