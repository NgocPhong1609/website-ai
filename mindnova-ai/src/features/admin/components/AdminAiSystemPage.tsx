...
export function AdminAiSystemPage() {
  ...
  return <div className="space-y-4 px-4 pb-5 pt-2.5 text-[#2C3039] [font-family:var(--font-admin-body)] lg:px-6">
    <header className="rounded-2xl border border-[#FAF7F2]/20 bg-[linear-gradient(120deg,#1e293b_0%,#0f766e_55%,#0369a1_100%)] px-5 py-4 text-white shadow-md">
    ...
    {loading && !data ? <div role="status" aria-busy="true" className="space-y-4">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <label className="text-xs text-slate-600">Khoảng thời gian
          <select value={period} disabled={loading || saving} onChange={(event) => void refresh(event.target.value as AiPeriod)} className="ml-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-[#2C3039]">
      ...
      {pendingPeriod && <div role="alertdialog" aria-labelledby="discard-title" aria-describedby="discard-description" className="rounded-2xl border border-orange-200 bg-orange-50 p-4" onKeyDown={(event) => { if (event.key === "Escape") setPendingPeriod(null); }}>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <button type="button" onClick={() => void refresh(pendingPeriod, true)} className="rounded-xl bg-[#C0392B] px-3 py-2 text-sm text-white">Bỏ thay đổi và tải lại</button>
        </div>
      </div>}
      ...
      <form ref={form} noValidate onSubmit={(event) => { event.preventDefault(); void save(); }} className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button type="submit" disabled={!dirty || loading || saving} className="rounded-xl bg-[#C0392B] px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">{saving ? "Đang lưu..." : "Lưu cấu hình"}</button>
