...
export function AdminUsersManagementPage() {
  ...
  return (
    <div className="space-y-6 p-6 lg:p-8 [font-family:var(--font-admin-body)]">
      <section className="rounded-[28px] border border-cyan-200/20 bg-[linear-gradient(120deg,#0b1d40_0%,#155e75_45%,#134e4a_100%)] p-6 text-white shadow-[0_30px_70px_-35px_rgba(7,18,45,0.8)]">
        <p className="text-xs uppercase tracking-[0.34em] text-cyan-100/70">User Management</p>
        <h1 className="mt-2 text-3xl font-semibold [font-family:var(--font-admin-head)]">Quản lý người dùng và phân quyền</h1>
        <p className="mt-2 max-w-3xl text-sm text-slate-100/90">Cấp quyền, khóa/xóa tài khoản Teacher, Student, Guest; theo dõi đăng nhập, thời gian học, lịch sử thao tác; duyệt hồ sơ giáo viên.</p>
      </section>
      ...
      <section className="rounded-2xl border border-cyan-100/80 bg-white/95 p-4">
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <button onClick={() => void loadAll()} className="rounded-xl bg-cyan-600 px-4 py-2 text-sm font-semibold text-white">Lọc dữ liệu</button>
        </div>
      </section>
            ...
            {teacherQueue.map((teacher) => (
              <div key={teacher.id} className="rounded-xl border border-slate-200 p-3">
                ...
                <div className="mt-2 flex gap-2">
                  <button onClick={() => void reviewTeacher(teacher.id, "approved")} className="rounded-lg bg-emerald-100 px-2 py-1 text-xs font-semibold text-emerald-800">Duyệt</button>
...
function StatCard({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-2xl border border-cyan-100/80 bg-white/95 p-4 shadow-[0_14px_32px_-25px_rgba(10,18,40,0.4)]">
