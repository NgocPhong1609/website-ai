...
export async function AdminModerationPage() {
 ...
 <p className="text-[10px] uppercase tracking-[0.35em] -[#FAF7F2]/65">Moderation & support</p>
 ...
 <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${item.severity === "high" ? "bg-rose-100 text-rose-700" : item.severity === "medium" ? "bg-amber-50 text-amber-700" : "bg-emerald-50 -[#2C3039]"}`}>
 {item.severity}
 </span>
 </td>
 <td className="px-4 py-3">
 <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${item.status === "open" ? "bg-sky-50 text-sky-700" : item.status === "in_review" ? "bg-violet-50 -[#C0392B]" : "bg-emerald-50 -[#2C3039]"}`}>
