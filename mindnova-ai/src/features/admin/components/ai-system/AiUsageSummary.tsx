...
const panel = "min-w-0 rounded-2xl border border-[#FAF7F2] bg-white p-4 shadow-sm";
  ...
  return <div className="space-y-4">
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {metrics.map((metric) => <section key={metric.title} aria-label={metric.title} className={panel}>
        ...
        <p className="mt-2 break-words text-2xl font-semibold text-[#2C3039]">{metric.value}</p>
    ...
    {!compact && <div className="grid gap-4 xl:grid-cols-[1.4fr_1fr]">
      <section aria-label="Xu hướng yêu cầu" className={panel}>
        <h2 className="font-semibold text-[#2C3039]">Yêu cầu theo ngày</h2>
      ...
      <section aria-label="Sử dụng theo nhà cung cấp" className={panel}>
        <h2 className="font-semibold text-[#2C3039]">Nhà cung cấp & mô hình</h2>
