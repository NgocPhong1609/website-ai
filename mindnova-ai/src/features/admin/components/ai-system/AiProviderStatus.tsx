...
export function AiProviderStatus({ providers }: { providers: AdminAiSystemData["providers"] }) {
  return <div className="grid gap-4 md:grid-cols-2">
    {([ ["primary", "AI chính"], ["backup", "AI dự phòng"] ] as const).map(([key, label]) => {
      ...
      return <section key={key} aria-label={label} className="min-w-0 rounded-2xl border border-[#E2E8F0] bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h2 className="font-semibold text-[#0F172A]">{label}</h2>
          <span className={`rounded-xl px-2 py-1 text-xs font-medium ${provider.configured ? "bg-emerald-50 text-emerald-800" : "bg-orange-50 text-orange-800"}`}>{provider.configured ? "Đã cấu hình" : "Chưa cấu hình"}</span>
        </div>
