...
export function AiPromptEditor({ prompts, errors, disabled, onChange }: {
  ...
  return <section className="rounded-2xl border border-[#E2E8F0] bg-white p-4 shadow-sm">
    <h2 className="text-lg font-semibold text-[#0F172A]">System Prompt</h2>
    <div className="mt-4 grid gap-4 xl:grid-cols-2">
      {([ ["ai_tro_giang", "AI Trợ giảng"], ["ai_cham_bai", "AI Chấm bài"] ] as const).map(([name, label]) => <div key={name} className="min-w-0">
        ...
        <textarea id={name} value={prompts[name]} rows={7} disabled={disabled}
          ...
          className="mt-2 w-full resize-y rounded-xl border border-slate-200 px-3 py-2 text-sm leading-6 focus:border-[#3B82F6] focus:outline-none focus:ring-2 focus:ring-blue-100 disabled:bg-slate-50" />
        <p id={`${name}-count`} className="text-right text-xs text-slate-600">{Array.from(prompts[name]).length}/4000 ký tự</p>
        {errors?.[name] && <p id={`${name}-error`} className="mt-1 text-xs text-rose-700">{errors[name]}</p>}
      </div>)}
    </div>
  </section>;
}
