...
export function AiPackageEditor({ packages, errors, disabled, onChange }: {
  ...
  return <section className="space-y-3">
    <h2 className="text-lg font-semibold text-[#0F172A]">Hạn mức gói AI</h2>
    ...
    <div className="grid gap-4 md:grid-cols-2">
      {([ ["free", "Free", 2000], ["premium", "Premium", 10000] ] as const).map(([tier, title, max]) => <fieldset key={tier} disabled={disabled} className="min-w-0 rounded-2xl border border-[#E2E8F0] bg-white p-4 shadow-sm">
        <legend className="px-1 font-semibold text-[#0F172A]">{title}</legend>
        {([ ["daily_requests", "Yêu cầu / ngày"], ["daily_tokens", "Token / ngày"] ] as const).map(([field, label]) => {
          ...
          return <div key={field} className="mt-3">
            ...
            <input id={id} type="number" min={1} max={field === "daily_requests" ? max : undefined} step={1}
              ...
              className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-[#3B82F6] focus:outline-none focus:ring-2 focus:ring-blue-100 disabled:bg-slate-50" />
