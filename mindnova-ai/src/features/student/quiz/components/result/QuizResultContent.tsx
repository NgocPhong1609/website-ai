        ...
        <div className="max-w-[860px] mx-auto space-y-8">
          ...
          <div className="flex items-center justify-between text-xs text-[#8A8478]">
            <div className="flex items-center gap-2">
              <Link href="/practice" className="hover:text-[#C0392B] transition-colors">Trung tâm thực chiến</Link>
              <span></span>
              <span className="text-[#2C3039] font-medium">Báo cáo kết quả bài thi AI</span>
            </div>
            <span className="px-3 py-1 rounded-full bg-[#FAF7F2] text-[#C0392B] font-semibold border border-[#C0392B]/15">
              Mã đề: #{aiQuiz.id}
            </span>
          </div>

          {/* Score Banner */}
          <div className={`p-8 rounded-3xl border shadow-sm flex flex-col md:flex-row items-center justify-between gap-6 ${isPassed
            ? " from-[#ECFDF5] via-white to-[#F0FDFA] border-[#2C3039]/30"
            : " from-[#FFF1F2] via-white to-[#FEF2F2] border-[#C0392B]/30"
            }`}>
            <div className="space-y-2 text-center md:text-left">
              <span className={`text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider inline-block ${isPassed ? "bg-[#FAF7F2] text-[#065F46]" : "bg-[#FAF7F2] text-[#C0392B]"
                }`}>
                {isPassed ? " Đạt Chuẩn Đánh Giá" : " Cần Cố Gắng Thêm"}
              </span>
              <h1 className="text-2xl sm:text-3xl font-extrabold text-[#2C3039]">{aiQuiz.title}</h1>
              <p className="text-xs sm:text-sm text-[#8A8478]">
                Chủ đề: <strong className="text-[#C0392B]">{aiQuiz.topic}</strong> • Độ khó: <strong>{aiQuiz.difficulty}</strong>
              </p>
            </div>

            <div className="flex flex-col items-center">
              <div className={`text-5xl font-black ${isPassed ? "text-[#2C3039]" : "text-[#C0392B]"}`}>
                {aiQuiz.score}%
              </div>
              <span className="text-xs font-semibold text-[#8A8478] mt-1">
          ...
          <div className="space-y-5">
            <div className="flex items-center justify-between px-1">
              <h2 className="text-base font-bold text-[#2C3039]"> Chi tiết bài làm &amp; Hướng dẫn giải</h2>
              <span className="text-xs text-[#8A8478]">Tổng cộng {aiQuiz.questions_count} câu hỏi</span>
            </div>
            ...
            {aiQuiz.questions_data?.map((q: any, idx: number) => {
              ...
              return (
                <div key={q.id || idx} className="p-6 rounded-2xl bg-white border border-[#FAF7F2] shadow-xs space-y-4">
                  <div className="flex items-start justify-between gap-4">
                    <h3 className="text-sm sm:text-base font-bold text-[#2C3039] leading-relaxed">
                      Câu {idx + 1}: {q.question}
                    </h3>
                    <span className={`text-xs font-bold px-3 py-1 rounded-full shrink-0 ${isCorrect ? "bg-[#FAF7F2] text-[#065F46]" : "bg-[#FAF7F2] text-[#C0392B]"
                      }`}>
                  ...
                  {isEssayOrFill ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                      {q.options?.map((opt: string, optIdx: number) => {
                        ...
                        let style = "bg-[#F8FAFC] border-[#FAF7F2] text-[#2C3039]";
                        if (isRightKey) {
                  ...
                  <div className="pt-2">
                    <div className="p-4 rounded-xl bg-[#FAF7F2]/70 border border-[#C0392B]/20 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-[#C0392B] flex items-center gap-1.5">
          ...
          {/* Action Footer */}
          <div className="p-6 rounded-3xl bg-white border border-[#FAF7F2] shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
            <Link href="/practice" className="w-full sm:w-auto">
              <button type="button" className="w-full sm:w-auto px-6 py-3 rounded-xl border border-[#FAF7F2] text-xs sm:text-sm font-semibold text-[#2C3039] hover:bg-[#F8FAFC] cursor-pointer">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
              <Link href={`/practice/quiz/question?aiQuizId=${aiQuiz.id}`} className="w-full sm:w-auto">
                <button type="button" className="w-full sm:w-auto px-6 py-3 rounded-xl bg-[#FAF7F2] text-[#C0392B] border border-[#C0392B]/30 text-xs sm:text-sm font-bold hover:bg-[#E0E7FF] transition-all flex items-center justify-center gap-2 cursor-pointer">
                  <span></span>
              ...
              <button
                ...
                className="w-full sm:w-auto px-6 py-3 rounded-xl bg-[#C0392B] text-white text-xs sm:text-sm font-bold shadow-md hover:opacity-95 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
              >
                {isGeneratingSimilar ? (
                  <>
    ...
    <div className="flex-1 overflow-y-auto bg-[#F8F9FC] min-h-screen relative">
      ...
      {isReviewModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 sm:p-6 animate-fadeIn">
          <div className="bg-white w-full max-w-4xl rounded-3xl border border-[#FAF7F2] shadow-2xl max-h-[90vh] flex flex-col overflow-hidden animate-scaleUp">

            <div className="p-6 from-[#EEF2FF] via-[#F3F4FC] to-[#EAF8F5] border-b border-[#FAF7F2] flex items-center justify-between shrink-0">
              <div className="space-y-1">
                <div className="inline-flex items-center gap-2 px-3 py-0.5 rounded-full bg-white text-xs font-semibold text-[#C0392B] border border-[#C0392B]/20">
                  <span> Soát Lỗi Chi Tiết từ Gia Sư AI Nova</span>
                </div>
                <h3 className="text-lg sm:text-xl font-bold text-[#2C3039]">Bảng Đối Chiếu Đáp Án, Bài Làm Tự Luận &amp; Nhận Xét Rubric</h3>
              </div>
              <button
                type="button"
                onClick={() => setIsReviewModalOpen(false)}
                className="w-10 h-10 rounded-xl bg-white text-[#8A8478] hover:text-[#2C3039] hover:bg-[#F0F2F8] border border-[#FAF7F2] flex items-center justify-center font-semibold text-lg transition-colors cursor-pointer"
                title="Đóng cửa sổ"
              >
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 bg-[#F8FAFC]">
              <div className="flex flex-wrap gap-2 pb-2 border-b border-[#FAF7F2]">
                <button
                  onClick={() => setSelectedFilter("all")}
                  className={`px-3.5 py-1.5 rounded-xl text-xs font-semibold transition-colors cursor-pointer ${selectedFilter === "all" ? "bg-[#C0392B] text-white" : "bg-white text-[#8A8478] border border-[#FAF7F2]"}`}
                >
                  Tất cả ({questionResultsList.length > 0 ? questionResultsList.length : 10} Câu)
                </button>
                <button
                  onClick={() => setSelectedFilter("mc")}
                  className={`px-3.5 py-1.5 rounded-xl text-xs font-semibold transition-colors cursor-pointer ${selectedFilter === "mc" ? "bg-[#2C3039] text-white" : "bg-white text-[#2C3039] border border-[#2C3039]/30"}`}
                >
              ...
              <div className="space-y-5">
                {filteredQuestions.length > 0 ? (
                  filteredQuestions.map((item) => (
                    <div key={item.question_id} className="bg-white rounded-2xl p-5 border border-[#FAF7F2] shadow-2xs space-y-4 hover:border-[#C0392B]/30 transition-colors">
                      <div className="flex items-center justify-between gap-3 flex-wrap">
                        <span className="text-xs font-semibold px-3 py-1 rounded-full bg-[#FAF7F2] text-[#C0392B] border border-[#C0392B]/15">
                          Câu #{item.order} • {item.type === 'essay' ? '️ Tự luận AI' : ' Trắc nghiệm'}
                        </span>

                        <div className="flex items-center gap-2">
                          <span className={`text-xs font-semibold px-3 py-1 rounded-full border ${item.score >= (item.max_score * 0.8)
                      ...
                      {item.image_url && (
                        <img
                          ...
                          className="max-h-72 w-full rounded-xl border border-[#FAF7F2] bg-white object-contain"
                        />
                      )}

                      {item.type === 'essay' ? (
                        <div className="space-y-3 pt-1">
                          <div className="p-4 rounded-xl bg-[#F8FAFC] border border-[#FAF7F2] space-y-1">
                            <span className="text-[11px] font-bold uppercase tracking-wider text-[#C0392B]"> Bài làm tự luận của bạn:</span>
                          ...
                          {item.sample_answer && (
                            <div className="p-4 rounded-xl bg-[#EAF8F5]/60 border border-[#2C3039]/20 space-y-1">
                              <span className="text-[11px] font-bold uppercase tracking-wider text-[#2C3039]"> Đáp án tham khảo mẫu:</span>
                          ...
                          <div className="p-4 rounded-xl from-[#EEF2FF]/80 via-[#F3F4FC] to-[#EAF8F5]/80 border border-[#C0392B]/20 space-y-2">
                            ...
                            {item.ai_analysis?.matched_points && item.ai_analysis.matched_points.length > 0 && (
                              <div className="pt-2 border-t border-[#FAF7F2] space-y-1">
                                <span className="text-[10px] font-bold text-[#2C3039] uppercase"> Ý đạt điểm (Matched Points):</span>
                        ...
                        <div className="space-y-3 pt-1">
                          {item.selection_type === 'multiple_choice' && item.answer_options ? (
                                ...
                                const style = state === 'selected-correct'
                                  ...
                                  : state === 'selected-incorrect'
                                    ...
                                    : state === 'missed-correct'
                                      ...
                                      : 'bg-[#F8FAFC] border-[#FAF7F2] text-[#8A8478]';
                            ...
                            <>
                              <div className={`p-3.5 rounded-xl border text-xs sm:text-sm font-semibold flex items-center gap-2 ${item.is_correct
                              ...
                              {item.answer_options
                                ...
                                .map((answer) => (
                                  <img
                                    ...
                                    alt={`Hình minh họa đáp án: ${answer.content}`}
                                    className="max-h-40 w-full rounded-lg border border-[#FAF7F2] bg-white object-contain"
                                  />
                                ))}
                            </>
                          )}

                          <div className="bg-[#F8FAFC] p-4 rounded-xl border border-[#FAF7F2] space-y-1.5">
                  ...
                  <div className="text-center py-12 text-[#8A8478] text-sm">
            ...
            <div className="p-4 bg-white border-t border-[#FAF7F2] flex items-center justify-end gap-3 shrink-0">
              <button
                type="button"
                onClick={() => setIsReviewModalOpen(false)}
                className="px-6 py-2.5 bg-[#C0392B] text-white rounded-xl font-semibold text-xs sm:text-sm hover:opacity-95 transition-all shadow-2xs cursor-pointer"
              >
      ...
      <div className="max-w-[1020px] mx-auto px-6 py-8 pb-24 space-y-7">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs text-[#8A8478]">
            <Link href="/practice" className="hover:text-[#C0392B] transition-colors text-decoration-none">Trung tâm thực chiến</Link>
            <span></span>
            <span className="text-[#2C3039] font-medium">Báo cáo kiểm tra Năng lực AI</span>
          </div>
          <span className="text-xs font-medium px-3 py-1 rounded-full bg-[#FAF7F2] text-[#C0392B] border border-[#C0392B]/15">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
          <div className="md:col-span-7 from-white via-[#FAFBFF] to-[#F3F4FC] rounded-2xl border border-[#FAF7F2] shadow-2xs relative overflow-hidden p-8 flex flex-col items-center justify-center text-center transition-all duration-300 hover:shadow-sm">
            ...
            <div className="relative z-10 mt-2 space-y-1">
              <p className="text-xs font-semibold tracking-wider text-[#8A8478] uppercase">Điểm Thành Tích Chung (Thang Điểm 10)</p>
              <div className="flex items-baseline justify-center gap-1.5 my-2">
                <span className="text-6xl sm:text-7xl font-bold bg-[#C0392B] bg-clip-text text-transparent tracking-tight">
                  {displayScore10}
                </span>
                <span className="text-2xl font-semibold text-[#8A8478]">/10.0</span>
              </div>
            </div>

            <p className="text-xs text-[#8A8478] mt-1">Bài thi: <span className="font-medium text-[#2C3039]">{displayData.quiz_title || "Kiểm tra thực chiến"}</span></p>

            <div className="relative z-10 grid grid-cols-2 gap-8 w-full max-w-xs mt-7 pt-5 border-t border-[#FAF7F2]">
              <div>
                <p className="text-xs font-normal text-[#8A8478] mb-1">Tỷ lệ chính xác</p>
                <p className="text-sm sm:text-base font-semibold text-[#2C3039] flex items-center justify-center gap-1">
                  <span className="text-[#2C3039]"> {displayData.correct_count}</span> / {displayData.total_questions} Câu
                </p>
              </div>
              <div className="border-l border-[#FAF7F2]">
                <p className="text-xs font-normal text-[#8A8478] mb-1">Thời gian làm bài</p>
                <p className="text-sm sm:text-base font-semibold text-[#2C3039]">
          ...
          <div className="md:col-span-5 bg-white rounded-2xl border border-[#FAF7F2] shadow-2xs p-6 flex flex-col justify-between transition-all duration-300 hover:shadow-sm hover:border-[#C0392B]/30">
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-[#F0F2F8] pb-3.5">
                <div className="flex items-center gap-2.5 text-[#C0392B]">
                  ...
                  <h3 className="font-semibold text-base text-[#2C3039]">Nhận xét từ Gia sư AI</h3>
                </div>
                <span className="text-[11px] bg-[#EAF8F5] text-[#2C3039] px-2.5 py-0.5 rounded-full font-medium border border-[#2C3039]/15">
                  MindNova Co-Pilot
                </span>
              </div>

              <p className="text-xs sm:text-sm text-[#2C3039] leading-relaxed font-normal bg-[#F8FAFC] p-4 rounded-xl border border-[#FAF7F2]">
            ...
            <div className="pt-4 border-t border-[#F0F2F8] mt-4 space-y-1.5">
              <p className="text-[11px] font-semibold tracking-wide text-[#8A8478] uppercase"> Chiến thuật tiếp theo</p>
              <p className="text-xs sm:text-sm font-semibold text-[#C0392B] bg-[#FAF7F2]/50 p-3 rounded-xl border border-[#C0392B]/15">
        ...
        {/* Topic Performance */}
        <div className="bg-white rounded-2xl border border-[#FAF7F2] shadow-2xs p-6 sm:p-7 transition-all duration-300 hover:shadow-sm">
          <div className="flex items-center justify-between border-b border-[#F0F2F8] pb-4 mb-6">
            <div>
              <h3 className="text-base sm:text-lg font-semibold text-[#2C3039]">Phân tích độ thành thạo theo chủ đề (Topic Mastery)</h3>
              <p className="text-xs text-[#8A8478] mt-0.5">Trí tuệ nhân tạo phân rã mức độ thông hiểu kỹ thuật từ bài thi của bạn</p>
            </div>
            <span className="text-xs text-[#C0392B] bg-[#FAF7F2]/60 px-3 py-1 rounded-full font-medium hidden sm:inline-block border border-[#C0392B]/15">
              3 Chủ đề cốt lõi
            </span>
          </div>
          ...
          <div className="flex flex-col gap-6">
            {(displayData.topic_performance || []).map((t, idx) => (
              <div key={t.id || idx} className="p-4 rounded-xl bg-[#F8FAFC] border border-[#FAF7F2] hover:border-[#C0392B]/20 transition-colors space-y-2.5">
                ...
                <div className="h-2 w-full bg-[#EAEAF4] rounded-full overflow-hidden p-0.5">
                  <div
                    className={`h-full rounded-full transition-all duration-700 ${t.score_percentage >= 80 ? " bg-[#2C3039] " : t.score_percentage >= 60 ? " bg-[#C0392B] " : " from-[#F43F5E] to-[#E11D48]"}`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div
            ...
            className="bg-white rounded-2xl p-6 border border-[#FAF7F2] shadow-2xs transition-all duration-300 hover:shadow-md hover:border-[#C0392B]/50 hover:-translate-y-0.5 flex flex-col justify-between group cursor-pointer"
          >
            <div>

              <h4 className="font-semibold text-base text-[#2C3039] mt-5 mb-2 group-hover:text-[#C0392B] transition-colors">
            ...
            <div className="mt-6 pt-3 border-t border-[#F0F2F8] flex items-center justify-between text-xs font-semibold text-[#C0392B] group-hover:text-[#C0392B]">
          ...
          <Link href={`/practice/quiz/question?lessonId=${targetModuleId}`} className="text-decoration-none block">
            <div className="h-full bg-white rounded-2xl p-6 border border-[#FAF7F2] shadow-2xs transition-all duration-300 hover:shadow-md hover:border-[#2C3039]/50 hover:-translate-y-0.5 flex flex-col justify-between group cursor-pointer">
              <div>

                <h4 className="font-semibold text-base text-[#2C3039] mt-5 mb-2 group-hover:text-[#2C3039] transition-colors">
              ...
              <div className="mt-6 pt-3 border-t border-[#F0F2F8] flex items-center justify-between text-xs font-semibold text-[#2C3039] group-hover:text-[#2C3039]">
          ...
          <Link href="/practice" className="text-decoration-none block">
            <div className="h-full bg-white rounded-2xl p-6 border border-[#FAF7F2] shadow-2xs transition-all duration-300 hover:shadow-md hover:border-[#C0392B]/50 hover:-translate-y-0.5 flex flex-col justify-between group cursor-pointer">
              <div>
        ...
        <div className="flex flex-wrap justify-center items-center gap-4 pt-4">
          ...
          <Link href={`/practice/quiz/question?lessonId=${targetModuleId}`} className="text-decoration-none">
            <button type="button" className="px-7 py-3 bg-white border border-[#FAF7F2] hover:bg-[#F8FAFC] text-[#2C3039] rounded-xl font-medium text-xs sm:text-sm shadow-2xs hover:border-[#C0392B]/30 transition-all cursor-pointer flex items-center gap-2">
              <span></span>
