...
function StatusBadge({ status }: { status: TransactionStatus }) {
  const config = STATUS_STYLES[status] || { text: status, clazz: "bg-gray-100 text-[#8A8478] border border-[#FAF7F2]" };
  return (
    <span
      className={twMerge(
...
function FilterDropdown({ value, onChange }: FilterDropdownProps) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1.5 z-30 w-44 rounded-xl bg-white border border-[#FAF7F2] shadow-[0_8px_24px_rgba(0,0,0,0.12)] py-1 overflow-hidden">
  ...
  return (
    <tr className="group hover:bg-[#F8FAFC]/80 transition-colors duration-150 border-b border-[#F4F5FB] last:border-b-0">
      ...
      <td className="pl-6 pr-4 py-4 whitespace-nowrap">
        <span className="inline-flex items-center px-2.5 py-1 rounded-lg bg-[#F4F5FD] text-[#C0392B] font-semibold text-xs border border-[#FAF7F2] select-all">
          {tx.invoiceId}
        </span>
      </td>
...
export function TransactionHistoryTable() {
  ...
  return (
    <div className="rounded-2xl bg-white border border-[#FAF7F2] shadow-2xs overflow-hidden transition-all duration-300 hover:shadow-sm">
      {/* Table header console */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 py-5 border-b border-[#FAF7F2] bg-[#F8FAFC]/50">
        <div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left min-w-[760px]">
          <thead>
            <tr className="border-b border-[#FAF7F2] bg-[#F8FAFC]">
              {[
