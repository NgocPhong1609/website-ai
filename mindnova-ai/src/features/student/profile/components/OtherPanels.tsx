...
function ActiveSessionsBox() {
    return (
        <div className="mt-2 rounded-2xl border border-[#FAF7F2] bg-[#F8FAFC] p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-2xs">
            ...
            <button
                ...
                className="shrink-0 px-4 py-2 rounded-xl bg-white hover:bg-[#FAF7F2] border border-[#FAF7F2] hover:border-[#C0392B]/30 text-xs font-semibold text-[#C0392B] transition-all duration-200 shadow-2xs cursor-pointer focus:outline-none"
            >
...
export function SecurityPanel() {
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="border-b border-[#FAF7F2] pb-4 flex items-center justify-between">
                <div>
            ...
            <hr className="border-t border-[#FAF7F2] mt-1" />
                ...
                {toggles.map(({ id, label, description, value, handler }) => (
                    ...
                    >
                        ...
                        <button
                            ...
                            disabled={isSaving}
                            className={`relative w-12 h-6.5 rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-[#C0392B]/30 shrink-0 ${isSaving ? "cursor-not-allowed" : "cursor-pointer"} ${value ? " bg-[#C0392B] " : "bg-[#CBD5E1]"
                                }`}
                        >
                            <span
                                className={`absolute top-1 left-1 w-4.5 h-4.5 rounded-full bg-white shadow-md transition-transform duration-200 ${value ? "translate-x-5.5" : "translate-x-0"
                                    }`}
                            />
                        </button>
                    </div>
