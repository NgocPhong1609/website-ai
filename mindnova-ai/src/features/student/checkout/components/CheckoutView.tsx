...
export function CheckoutView({ courseId }: { courseId: number }) {
 ...
 <div className="w-24 h-24 rounded-xl from-[#1E1B4B] shrink-0" />
 )}
 <div>
 <h3 className="text-base font-bold text-[#2C3039] leading-snug mb-1">
 {header_info.title}
 </h3>
 <span className="inline-block px-2.5 py-1 bg-[#FAF7F2] text-[#C0392B] text-xs font-semibold rounded-lg">
 {header_info.level}
 </span>
 </div>
 </div>
 ...
 <span className="font-semibold text-[#2C3039]">- 0 VND</span>
 </div>
 <div className="flex justify-between text-lg font-bold text-[#2C3039] pt-2 border-t border-[#E8E2D9]">
 <span>Tổng thanh toán</span>
 <span className="text-[#C0392B]">{isFree ? "Miễn phí" : `${Number(header_info.price).toLocaleString()} VND`}</span>
 </div>
 </div>
 </div>
 </div>
 ...
 <p className="text-[#8A8478] text-sm">Khóa học này hoàn toàn miễn phí.</p>
 <p className="text-[#8A8478] text-sm">Nhấn nút bên dưới để thêm khóa học vào tài khoản của bạn.</p>
 </div>
 <button
 type="button"
 onClick={handleCheckout}
 disabled={isProcessing}
 className="w-full py-3.5 rounded-xl text-sm font-bold text-white bg-[#C0392B] shadow-md hover:shadow-lg disabled:opacity-60 transition-all flex justify-center items-center gap-2"
 >
 ...
 onClick={() => router.back()}
 disabled={isProcessing}
 className="w-full mt-3 py-3 rounded-xl text-sm font-semibold text-[#8A8478] bg-white border border-[#E8E2D9] hover:bg-[#E8E2D9] transition-all"
 >
 Quay lại
 </button>
 </div>
