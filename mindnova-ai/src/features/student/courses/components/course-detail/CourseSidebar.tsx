...
function ProgressCard({ progress }: { progress?: CourseDetailProgressCard }) {
    ...
    return (
        <div className="bg-white rounded-xl border border-[#FAF7F2] p-5 shadow-sm hover:border-[#B8B0A3] transition-all duration-300">
            ...
            <div className="w-full h-1.5 bg-[#F5F0E8] rounded-full overflow-hidden p-0 border border-[#FAF7F2] mt-4">
                <div
            ...
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-[#FAF7F2] text-xs font-medium text-[#8A8478]">
                ...
                <span className="font-bold text-[#2C3039] bg-[#F5F0E8] px-2.5 py-1 rounded-md border border-[#FAF7F2]">
                    {completed}/{total} Bài
                </span>
            </div>
        </div>
...
function AiInsightCard({ aiInsight }: { aiInsight?: CourseDetailAIInsight }) {
    ...
    return (
        <div className="bg-[#FEFCF9] rounded-xl border border-[#FAF7F2] p-5 relative overflow-hidden transition-all duration-300 hover:border-[#B8B0A3]">
            ...
            <div className="flex items-center justify-between mb-4">
                ...
                <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-white text-[11px] font-bold text-[#2C3039] border border-[#FAF7F2]">
                    <span>{statusTag}</span>
                </div>
            </div>
            ...
            {suggestionText && (
                <div className="mt-3 p-3 rounded-lg bg-white border border-[#FAF7F2] text-xs text-[#2C3039] flex items-start gap-2.5">
                    <div>
...
function ResourcesCard({ resources = [] }: { resources?: CourseDetailResourceItem[] }) {
    ...
    return (
        <div className="bg-white rounded-xl border border-[#FAF7F2] p-5 hover:border-[#B8B0A3] transition-all">
            ...
            <div className="space-y-2.5">
                {displayList.map((res, idx) => (
                    <button
                        ...
                        className="w-full text-left p-3 rounded-lg border border-[#FAF7F2] hover:border-[#2C3039] hover:bg-[#F5F0E8] transition-all duration-150 flex items-center justify-between gap-3 group cursor-pointer"
                    >
                        <div className="flex items-center gap-3 min-w-0">
                            <span className="text-xs font-bold text-[#2C3039] bg-white px-2 py-1 rounded border border-[#FAF7F2] uppercase w-10 text-center">
                                {res.type || "DOC"}
                            </span>
...
function InstructorCard({ instructor }: { instructor?: CourseDetailInstructor & { is_verified?: boolean; avatar_url?: string } }) {
    ...
    return (
        <div className="bg-white rounded-xl border border-[#FAF7F2] p-5 flex flex-col items-center text-center relative overflow-hidden group hover:border-[#B8B0A3] transition-all">
            <div className="relative z-10 mb-3 mt-3">
                <Avatar
                    src={avatarSrc}
                    fallback={name}
                    size="xl"
                    className="w-20 h-20 text-xl font-bold border border-[#FAF7F2] bg-[#F5F0E8] text-[#2C3039]"
                />
            </div>

            <span className="text-[11px] font-bold text-[#2C3039] bg-[#F5F0E8] px-2.5 py-0.5 rounded-full border border-[#FAF7F2] mb-2">
                Giảng viên Chủ trì
            </span>
            ...
            <p className="text-xs text-[#8A8478] leading-relaxed max-w-xs font-normal mb-5 border-t border-[#FAF7F2] pt-4">
                {bio}
            </p>
            ...
            <button
                ...
                className="w-full py-2.5 rounded-lg text-xs font-bold text-[#2C3039] bg-white hover:bg-[#F5F0E8] border border-[#FAF7F2] hover:border-[#B8B0A3] transition-all cursor-pointer"
            >
...
function EnrollCard({ price, courseId }: { price?: number, courseId?: string | number }) {
    ...
    return (
        <div className="bg-white rounded-xl border border-[#FAF7F2] p-5 hover:border-[#B8B0A3] transition-all duration-300">
            <div className="flex items-center justify-between gap-2 mb-3 pb-3 border-b border-[#FAF7F2]">
                <div>
