    ...
    const content = (
        <div className={twMerge(
            ...
            isCurrent
                ...
                : isCompleted
                    ? "bg-[#FEFCF9] border-[#FAF7F2] hover:bg-[#FAF7F2] hover:border-[#B8B0A3]"
                    : "bg-[#F5F0E8] border-[#FAF7F2] hover:bg-[#FAF7F2] opacity-80"
        )}>
                ...
                <div className={twMerge(
                    ...
                    isCompleted ? "bg-[#2C3039] border-[#2C3039] text-white" :
                        isCurrent ? "bg-[#2C3039] border-[#2C3039] text-white" :
                            "bg-white border-[#FAF7F2] text-[#8A8478]"
                )}>
                    {isCompleted && ""}
                    {isCurrent && "▶"}
                    {isLocked && ""}
            ...
            <div className="shrink-0 flex items-center gap-2.5 ml-4">
                ...
                <span className={twMerge(
                    ...
                    isCurrent
                        ...
                        : "text-[#8A8478] bg-white border-[#FAF7F2]"
                )}>
                    {lesson.duration}
                </span>
            </div>
...
export function CurriculumAccordion({ modules = [], courseId = 1 }: { modules?: CourseDetailModuleItem[], courseId?: string | number }) {
    ...
    return (
        <div className="bg-white border border-[#FAF7F2] rounded-xl p-6 sm:p-7 shadow-sm">
            {/* Header section */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 mb-6 border-b border-[#FAF7F2]">
                <div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                    <div className="px-3.5 py-1.5 rounded-lg border border-[#FAF7F2] bg-[#FAF7F2] text-xs font-bold text-[#8A8478]">
                    ...
                    <button
                        ...
                        className="text-xs font-bold text-[#8A8478] hover:text-[#2C3039] px-2.5 py-1.5 rounded bg-white border border-[#FAF7F2] hover:bg-[#F5F0E8] transition-colors cursor-pointer"
                    >
                ...
                {modules && modules.length > 0 ? (
                    modules.map((module, modIdx) => {
                        ...
                        return (
                            <div
                                key={modKey}
                                className={`rounded-xl border transition-all duration-200 overflow-hidden ${isExpanded ? "border-[#B8B0A3] bg-[#FEFCF9]" : "border-[#FAF7F2] bg-white hover:border-[#B8B0A3]"
                                    }`}
                            >
                                {/* Module Toggle Bar */}
                                <button
                                ...
                                >
                                    <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                                        <div className="w-10 h-10 rounded border border-[#FAF7F2] bg-white text-[#2C3039] font-bold text-xs sm:text-sm flex items-center justify-center shrink-0 font-serif">
                                    ...
                                    <div className="flex items-center gap-4 shrink-0">
                                        <span className={`text-[11px] font-bold px-2.5 py-1 rounded border hidden sm:inline-block ${completedInMod === totalInMod && totalInMod > 0
                                            ? "bg-[#E8F6F3] text-[#2C3039] border-[#2C3039]/20"
                                            : "bg-white text-[#8A8478] border-[#FAF7F2]"
                                            }`}>
                                            {completedInMod}/{totalInMod} Đã học
                                        </span>
                                        <div className="w-8 h-8 rounded border border-[#FAF7F2] bg-white text-[#2C3039] flex items-center justify-center shrink-0 font-bold">
                                            {isExpanded ? "-" : "+"}
                                        </div>
                                    </div>
                                </button>
                                ...
                                {isExpanded && (
                                    <div className="p-4 pt-0 space-y-2.5 border-t border-[#FAF7F2]">
