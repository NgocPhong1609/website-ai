...
function CourseReviewSection({ courseId }: { courseId: string | number }) {
    ...
    return (
        <section className="mt-8 w-full rounded-2xl border border-[#FAF7F2] bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <div className="rounded-lg border border-[#FAF7F2] bg-[#FAF7F2] px-4 py-2 text-sm font-semibold text-[#2C3039]">
                    Đánh giá: {averageRating.toFixed(1)} / 5
                </div>
            </div>

            <form onSubmit={onSubmit} className="mt-6 rounded-xl border border-[#FAF7F2] bg-[#FAF7F2] p-4">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    {[1, 2, 3, 4, 5].map((star) => (
                        <button
                            ...
                            className={`w-8 h-8 rounded-md text-sm font-bold border transition-colors ${star <= rating
                                ? "bg-[#2C3039] text-white border-[#2C3039]"
                                : "bg-white text-[#8A8478] border-[#FAF7F2] hover:border-[#B8B0A3]"
                                }`}
                ...
                <textarea
                    ...
                    className="mt-4 w-full resize-none rounded-xl border border-[#FAF7F2] bg-white px-4 py-3 text-sm text-[#2C3039] placeholder:text-[#B8B0A3] outline-none transition focus:border-[#B8B0A3]"
                />
            ...
            <div className="mt-6 space-y-4">
                {isReviewsLoading ? (
                    ...
                    reviews.map((review) => (
                        <div key={review.id} className="rounded-xl border border-[#FAF7F2] bg-[#FEFCF9] p-4">
                            <div className="flex items-start justify-between gap-3">
                                ...
                                <div className="flex items-center gap-2">
                                    <div className="rounded-md border border-[#FAF7F2] bg-white px-2 py-1 text-xs font-bold text-[#2C3039]">
                            ...
                            {editingReviewId === review.id ? (
                                <form onSubmit={(e) => handleEditSubmit(e, review.id)} className="mt-4 flex flex-col gap-3">
                                    <div className="flex items-center gap-2 mb-1">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <button
                                                ...
                                                className={`w-7 h-7 rounded text-xs font-bold border transition-colors ${star <= editRating
                                                    ? "bg-[#2C3039] text-white border-[#2C3039]"
                                                    : "bg-white text-[#8A8478] border-[#FAF7F2] hover:border-[#B8B0A3]"
                                                    }`}
                                                aria-label={`Chọn ${star} sao`}
                                    ...
                                    <textarea
                                        ...
                                        rows={3}
                                        className="w-full resize-none rounded-xl border border-[#FAF7F2] bg-white px-4 py-3 text-sm text-[#2C3039] outline-none focus:border-[#B8B0A3]"
                                    />
                                    <div className="flex justify-end gap-2">
                                        <button
                                            type="button"
                                            onClick={() => setEditingReviewId(null)}
                                            className="rounded-lg border border-[#FAF7F2] bg-white px-4 py-2 text-xs font-bold text-[#8A8478] hover:bg-[#F5F0E8]"
                                        >
                                            Hủy
                                        </button>
                                        <button
                                ...
                                <>
                                    ...
                                    <div className="mt-3 flex items-center justify-end gap-3 pt-3 border-t border-[#FAF7F2]">
                                        <button
