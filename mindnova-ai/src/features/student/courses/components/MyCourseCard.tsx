...
export function MyCourseCard({ course }: { course: MyCourse }) {
 ...
 <div className={twMerge("px-2.5 py-1 rounded-md bg-white/95 backdrop-blur-md text-[10px] font-bold shadow-sm uppercase tracking-wider flex items-center gap-1 border border-white/20", statusBadgeStyle)}>
 {isCompleted ? "Đã hoàn tất" : isNotStarted ? "Chưa bắt đầu" : "Đang học"}
 </div>
 </div>

 {course.isAiRecommended && (
 <div className="absolute top-3.5 right-3.5 flex items-center gap-1 px-2.5 py-1 rounded-md bg-white/95 backdrop-blur-md text-amber-500 text-[10px] font-bold border border-white/20 shadow-sm z-10 uppercase tracking-wider">
 ...
 <h3 className="text-base sm:text-lg font-bold text-slate-900 leading-snug line-clamp-1 group-hover/card:text-blue-600 group-hover/title:text-blue-600 transition-colors duration-300">
 {course.title}
 </h3>
 </Link>
 ...
 className={twMerge(
 "w-full py-2.5 px-4 rounded-md text-xs sm:text-sm font-semibold transition-all duration-200 flex items-center justify-center text-center text-decoration-none mt-auto",
 buttonStyle
 )}
 >
