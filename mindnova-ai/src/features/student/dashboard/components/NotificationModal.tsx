    ...
    <>
      ...
      <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 sm:p-6 pointer-events-none">
        <div
          role="dialog"
          aria-modal
          className="pointer-events-auto relative w-full max-w-[600px] bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col animate-in zoom-in-95 fade-in duration-200 border border-[#FAF7F2]"
        >
          {/* Header */}
          <div className="flex items-center px-6 py-4 border-b border-[#FAF7F2] relative bg-[#FAF7F2]">
            <button
              type="button"
              onClick={onClose}
              aria-label="Đóng"
          ...
          {/* Footer CTA */}
          {hasAction && (
            <div className="px-6 py-4 border-t border-[#FAF7F2] bg-[#FAF7F2] flex justify-end">
              <button
                type="button"
                onClick={() => {
                  onClose();
