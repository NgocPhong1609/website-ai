...
export function AdvancedRecommendationsSection({ recommendations = [] }: AdvancedRecommendationsSectionProps) {
    ...
    return (
        <section className="w-full flex flex-col gap-6">
            {/* Section Header */}
            <div className="w-full flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-3 border-b border-[#FAF7F2]">
                ...
                <div className="flex items-center gap-2 shrink-0">
                    <span className="text-xs font-semibold text-[#2C3039] bg-[#F5F0E8] px-3.5 py-1.5 rounded-full border border-[#FAF7F2]">
            ...
            <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {recommendations.map((rec) => {
                    ...
                    return (
                        <div
                            key={rec.id}
                            className="group bg-white border border-[#FAF7F2] rounded-xl flex flex-col justify-between w-full h-full hover:border-[#B8B0A3] transition-all duration-300 overflow-hidden"
                        >
                                    ...
                                    {displayTags.length > 0 && (
                                        <div className="flex flex-wrap gap-1.5 mt-auto pt-1">
                                            {displayTags.map((tag) => (
                                                <span
                                                    key={tag}
                                                    className="px-2.5 py-0.5 rounded-md bg-[#FAF7F2] group-hover:bg-[#F5F0E8] text-[#8A8478] text-xs font-medium transition-colors duration-200 border border-[#FAF7F2] truncate max-w-[140px]"
                                                >
                                                    #{tag}
                                                </span>
                                            ))}
