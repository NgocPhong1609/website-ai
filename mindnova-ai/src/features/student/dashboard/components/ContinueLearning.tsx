...
function CourseCard({ course }: { course: DashboardCourse }) {
  ...
  return (
    <div className={`group/card bg-white border rounded-xl flex flex-col justify-between h-full transition-all duration-300 overflow-hidden relative ${isAiPlan
      ? "border-[#D4A574]/50 hover:border-[#D4A574]"
      : "border-[#FAF7F2] hover:border-[#B8B0A3]"
      }`}>
      ...
      {/* Progress Bar */}
      <div className="w-full bg-[#F5F0E8] h-1.5 overflow-hidden p-0 border-b border-[#FAF7F2]">
        <div
  ...
  if (items.length === 0) {
    return (
      <section aria-labelledby="continue-learning-heading" className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-2 border-b border-[#FAF7F2] pb-3">
