...
export function ContextPanel({
    ...
    return (
        <div aria-label="AI Study Plan Context Inspector" className="w-full bg-white rounded-2xl border border-[#FAF7F2] shadow-sm p-6 sm:p-7 flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between">
                    ...
                    <span className="text-[11px] font-medium text-[#C0392B] bg-[#FEFCF9] px-2.5 py-0.5 rounded-md border border-[#FAF7F2]">{coreConcepts.length} Chủ đề</span>
                </div>
                    ...
                    {coreConcepts.map((concept) => {
                        ...
                        return (
                            <div
                                ...
                                className={`group p-4 rounded-2xl border transition-all duration-200 cursor-pointer ${isExpanded
                                    ? "bg-[#FEFCF9] border-[#C0392B] shadow-2xs"
                                    : "bg-[#FEFCF9] border-[#FAF7F2] hover:border-[#C0392B] hover:bg-white"
                                    }`}
                            >
                                <div className="flex items-center justify-between mb-1.5">
                                    ...
                                    {!isMastered && !isInProgress && (
                                        <span className="text-[11px] text-[#8A8478] font-medium bg-[#FAF7F2] px-2.5 py-0.5 rounded-full">
                                            Chờ học
                                        </span>
                                    )}
                                </div>
                                <p className="text-xs text-[#8A8478] leading-relaxed font-normal pl-4 border-l-2 border-[#FAF7F2] group-hover:border-[#C0392B] transition-colors">
                                    {concept.description}
                                </p>
                                {isExpanded && (
                                    <div className="mt-2.5 pt-2.5 border-t border-[#FAF7F2] flex flex-col gap-2.5">
                    ...
                    {lessonResources.map((res) => {
                        ...
                        return (
                            <a
                                key={res.id}
                                href={res.url || "#"}
                                className="group flex items-center justify-between p-3.5 sm:p-4 rounded-2xl bg-[#FEFCF9] hover:bg-[#FAF7F2] border border-[#FAF7F2] hover:border-[#C0392B] hover:-translate-y-0.5 transition-all duration-200"
                            >
                                <div className="flex items-center gap-3 min-w-0">
                                    <div
                                        className={`w-10 h-10 rounded-xl border flex items-center justify-center shadow-2xs shrink-0 transition-all duration-200 ${isVideo ? "text-[#2C3039] bg-[#FAF7F2] border-[#2C3039]" : "text-[#C0392B] bg-white border-[#C0392B]"
                                            }`}
                                    >
