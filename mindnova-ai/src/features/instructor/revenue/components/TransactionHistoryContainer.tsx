...
function Filters({
    ...
    return (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center gap-1 bg-white border border-[#FAF7F2] rounded-xl p-1 shadow-2xs overflow-x-auto">
                {tabs.map((tab) => (
                    <button
                        key={tab.id}
                        type="button"
            ...
            <div className="flex items-center gap-2">
                {activeDateRange === "custom" && (
                    <div className="flex items-center gap-2 animate-in fade-in slide-in-from-right-4 duration-300">
                        <input
                            ...
                            className="px-3 py-2 rounded-xl border border-[#FAF7F2] text-xs font-bold text-gray-700 bg-white shadow-2xs outline-none focus:border-[#C0392B] transition-colors"
                        />
                ...
                <div className="relative">
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 rounded-xl border border-[#FAF7F2] text-xs font-bold text-gray-700 bg-white shadow-2xs hover:border-[#C0392B] transition-colors cursor-pointer min-w-[170px] justify-between"
                    >
                        <div className="flex items-center gap-2">
                            <CalendarIcon />
                    ...
                    {isOpen && (
                        <div className="absolute right-0 top-full mt-1.5 w-44 bg-white border border-[#FAF7F2] rounded-xl shadow-lg z-20 py-1 overflow-hidden animate-in fade-in zoom-in-95 duration-100">
                            {dateOptions.map((opt) => (
                                <button
                                    key={opt.id}
                                    onClick={() => {
...
function TransactionTable({
    ...
    const getIconColor = (type: string) => {
        if (type === 'revenue') return "text-[#2C3039] bg-emerald-50 border-[#FAF7F2]";
        if (type === 'withdrawal') return "text-[#C0392B] bg-purple-50 border-[#FAF7F2]";
    ...
    const formatAmount = (amount: number, type: string) => {
        ...
        const color = type === 'withdrawal' || type === 'refund' ? 'text-[#2C3039] font-bold' : 'text-[#2C3039] font-black';
        return {
    ...
    if (isLoading) {
        return (
            <div className="bg-white rounded-2xl border border-[#FAF7F2] shadow-2xs p-12 flex flex-col items-center justify-center">
    ...
    if (isError || !data) {
        return (
            <div className="bg-white rounded-2xl border border-[#FAF7F2] shadow-2xs p-12 flex flex-col items-center justify-center text-rose-500">
    ...
    return (
        <div className="bg-white rounded-2xl border border-[#FAF7F2] shadow-2xs flex flex-col overflow-hidden">
            <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[750px]">
                    <thead>
                        <tr className="bg-[#FEFCF9]/70 border-b border-[#FAF7F2] text-[11px] font-black text-[#8A8478] uppercase tracking-wider">
            ...
            <div className="px-6 py-4 flex items-center justify-between border-t border-gray-100 bg-[#FEFCF9]/50">
                ...
                <div className="flex items-center gap-1">
                    <button
                        ...
                        className="w-8 h-8 flex items-center justify-center rounded-lg border border-[#FAF7F2] bg-white text-gray-400 disabled:opacity-50 hover:bg-[#FEFCF9] transition-colors cursor-pointer disabled:cursor-not-allowed">
                        <ChevronLeftIcon />
                    </button>
...
function BottomCards() {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-2xl border border-[#FAF7F2] p-5 shadow-2xs flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl bg-emerald-50 text-[#2C3039] flex items-center justify-center shrink-0 border border-[#FAF7F2]">
                    <TrendUpIcon size={22} />
                </div>
                <div>
                    <span className="text-xs font-bold text-[#8A8478] block uppercase">Thu nhập Tích lũy Tháng</span>
                    <span className="text-xl font-black text-[#2C3039] mt-0.5 block">{overview ? overview.total_revenue.toLocaleString('vi-VN') : '...'}đ</span>
                </div>
            </div>
            ...
            <div className="bg-indigo-50/50 rounded-2xl border border-[#FAF7F2] p-5 flex items-center justify-between gap-3 shadow-2xs">
                ...
                <Link
                    href="/instructor/revenue"
                    className="px-5 py-2.5 rounded-xl bg-[#C0392B] hover:bg-[#A93226] text-white text-xs font-extrabold shadow-sm transition-all shrink-0"
                >
                    Rút Ngay
                </Link>
            </div>
