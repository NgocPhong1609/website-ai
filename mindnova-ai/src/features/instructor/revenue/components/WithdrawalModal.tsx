...
export function WithdrawalModal({ isOpen, onClose, availableBalance, onSuccess }: WithdrawalModalProps) {
 ...
 <p className="text-xs -[#FAF7F2]">Hệ thống thanh toán tự động an toàn cho Giảng viên</p>
 </div>
 </div>
 <button
 type="button"
 ...
 <div className="p-4 rounded-xl bg-emerald-50 border -[#FAF7F2] flex flex-col justify-between">
 <div className="flex items-center justify-between">
 <span className="text-xs font-black -[#2C3039] uppercase">Số dư khả dụng</span>
 <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-md -[#FAF7F2] -[#2C3039]">Sẵn sàng rút</span>
 </div>
 <span className="text-lg font-black -[#2C3039] mt-1.5">{availableBalance.toLocaleString('vi-VN')}đ</span>
 </div>
 </div>
 ...
 className="px-2.5 py-1 rounded-lg bg-indigo-50 text-[#C0392B] font-extrabold text-xs hover:bg-[#C0392B] hover:text-white transition-all cursor-pointer border -[#FAF7F2] hover:border-[#C0392B]"
 >
 Tối đa
 </button>
 </div>
 ...
 payoutMethod === m.id ? "border-[#C0392B] bg-indigo-50/50 shadow-2xs" : "border-[#E8E2D9] bg-white hover:-[#C0392B]",
 !m.verified ? "opacity-50 cursor-not-allowed bg-[#FEFCF9]" : "cursor-pointer"
 )}
 >
 <div className="flex items-center justify-between">
 <span className="text-base">{m.icon}</span>
 <span className={twMerge("text-[10px] font-bold px-1.5 py-0.5 rounded-md", m.verified ? "-[#FAF7F2] -[#2C3039]" : "bg-gray-200 text-[#8A8478]")}>
 {m.verified ? "Đã duyệt" : "Chưa nối"}
 </span>
 </div>
 <div>
 <h5 className="text-xs font-black text-[#2C3039] leading-tight">{m.title}</h5>
 <p className="text-[10px] font-medium text-[#8A8478] truncate mt-0.5">{m.desc}</p>
 </div>
 </div>
 ))}
 </div>
 ...
 {statusMessage && (
 <div className={twMerge("p-3.5 rounded-xl font-bold text-xs flex items-center justify-between shadow-sm", isErrorMsg ? "bg-rose-100 text-rose-700" : "-[#2C3039] text-white")}>
