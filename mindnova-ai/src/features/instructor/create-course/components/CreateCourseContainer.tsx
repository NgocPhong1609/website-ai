...
export function CreateCourseContainer() {
  ...
  return (
    <div className="min-h-screen bg-[#F4F4F8] flex flex-col font-sans pb-16">
      {/* ── Header Bar ──────────────────────────────────────────────────────── */}
      <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-[#FAF7F2] px-6 py-4 shadow-2xs">
      ...
      <main className="max-w-6xl mx-auto w-full px-4 sm:px-6 pt-8 flex flex-col gap-6">
        ...
        <div className="mt-4 pt-4 border-t border-[#FAF7F2] flex items-center justify-between bg-white p-5 rounded-2xl shadow-2xs">
          <button
            type="button"
            onClick={handleBack}
            disabled={step === 1}
            className="flex items-center gap-2 px-4.5 py-2.5 rounded-xl text-xs font-bold text-gray-700 border border-[#FAF7F2] bg-white hover:bg-[#FEFCF9] disabled:opacity-40 disabled:pointer-events-none transition-all cursor-pointer shadow-2xs"
          >
            <span>← Quay lại</span>
          </button>
