...
export function TeacherProfileContainer() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-8">
      ...
      <div className="relative rounded-3xl border border-gray-200/80 bg-white p-6 md:p-8 shadow-sm overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-[#4648D4]/10 to-indigo-500/5 rounded-full blur-3xl -z-0 pointer-events-none" />

        <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start gap-6">
          {/* Avatar Upload */}
          <div className="relative group">
            <div className="w-24 h-24 md:w-28 md:h-28 rounded-full border-4 border-white shadow-lg overflow-hidden bg-gray-100 flex items-center justify-center text-3xl font-black text-[#4648D4]">
          ...
          <div className="flex-1 text-center md:text-left space-y-2">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 pt-1">
              <span className="px-3 py-1 rounded-full text-xs font-bold bg-indigo-50 text-[#4648D4] border border-indigo-100 uppercase tracking-wide">
                👨‍🏫 Giảng viên
              </span>
              <span
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm space-y-4">
            <div className="flex items-center gap-2">
              <span className="w-8 h-8 rounded-xl bg-[#4648D4]/10 text-[#4648D4] flex items-center justify-center font-black text-sm">
                ✦
              </span>
              <h2 className="text-base font-black text-gray-900">Xác minh chuyên môn</h2>
            </div>
            ...
            {isVerified ? (
              ...
              <div className="p-4 rounded-2xl bg-indigo-50/50 border border-indigo-100 space-y-3">
                ...
                <button
                  onClick={() => setIsModalOpen(true)}
                  className="w-full py-2.5 rounded-xl bg-[#4648D4] hover:bg-[#383AB8] text-white font-black text-xs shadow-md shadow-[#4648D4]/20 transition-all"
                >
          ...
          <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-base font-black text-gray-900">Bằng cấp & Chứng chỉ</h2>
              <button
                onClick={() => setIsModalOpen(true)}
                className="text-xs font-black text-[#4648D4] hover:underline"
              >
                + Thêm bằng cấp
              </button>
            </div>
            ...
            {profileData?.certificates?.length > 0 ? (
              <div className="space-y-3">
                {profileData.certificates.map((cert: any) => (
                  <div
                    key={cert.id}
                    className="p-3.5 rounded-2xl border border-gray-100 bg-gray-50/60 space-y-2 relative group"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h4 className="text-sm font-extrabold text-gray-900">{cert.certificate_name}</h4>
                    ...
                    {cert.certificate_image && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="block mt-2 rounded-xl overflow-hidden border border-gray-200 h-24 bg-gray-100"
                      >
                        <img
                    ...
                    {cert.verification_url && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-block text-[11px] font-bold text-[#4648D4] hover:underline"
                      >
                        🔗 Link xác minh chính thức ↗
                      </a>
                    )}

                    <div className="flex items-center justify-between pt-1 border-t border-gray-200/50 text-[11px] text-gray-400">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-3xl border border-gray-200 bg-white p-6 md:p-8 shadow-sm space-y-6">
            <div className="border-b border-gray-100 pb-4">
              <h2 className="text-lg font-black text-gray-900">Chỉnh sửa thông tin cá nhân</h2>
              <p className="text-xs font-semibold text-gray-500">
          ...
          <div className="rounded-3xl border border-gray-200 bg-white p-6 md:p-8 shadow-sm space-y-4">
            <h2 className="text-lg font-black text-gray-900">Danh sách Khóa học giảng dạy</h2>
            {profileData?.courses?.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {profileData.courses.map((course: any) => (
                  <div
                    key={course.id}
                    className="p-4 rounded-2xl border border-gray-100 bg-gray-50 flex items-center gap-3"
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <span className="text-[10px] font-bold uppercase text-[#4648D4]">
                        {course.status}
                      </span>
                    </div>
                  </div>
