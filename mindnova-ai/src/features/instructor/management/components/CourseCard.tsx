...
function EditButton({ courseId }: { courseId: string }) {
 ...
 className="absolute top-3 right-3 w-8 h-8 rounded-xl bg-white/90 backdrop-blur-sm flex items-center justify-center text-[#C0392B] shadow-sm hover:bg-white hover:text-[#4338CA] hover:scale-105 active:scale-95 transition-all duration-150 z-10 cursor-pointer border border-gray-100"
 >
 <PencilIcon />
 </Link>
 );
...
function CourseActionButtons({ courseId }: { courseId: string }) {
 ...
 className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold text-[#2C3039] bg-white border border-[#E8E2D9] hover:bg-[#FAF7F2] hover:text-[#C0392B] hover:-[#FAF7F2] active:scale-98 transition-all duration-150 cursor-pointer shadow-2xs"
 >
