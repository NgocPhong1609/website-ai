...
export function Step3GeneratingState() {
    ...
    return (
        <div className="p-12 bg-white rounded-3xl border border-[#FAF7F2] shadow-sm flex flex-col items-center justify-center text-center gap-6 animate-fadeIn min-h-[420px]">
            ...
            <div className="w-full max-w-md flex flex-col gap-3">
                ...
                <div className="flex items-center justify-between px-2">
                    {steps.map((_, idx) => (
                        <div
                            key={idx}
                            className={`h-2 flex-1 mx-1 rounded-full transition-all duration-500 ${idx <= currentProgressStep ? "bg-[#C0392B]" : "bg-gray-200"
                                }`}
                        />
                    ))}
                </div>
