...
export function QuestionCardMultipleChoice({
    ...
    return (
        <div
            className={`p-6 rounded-3xl bg-white border-2 transition-all duration-200 shadow-sm flex flex-col gap-4 ${isApproved ? "-[#2C3039]/50 bg-emerald-50/10 shadow-[0_4px_20px_rgba(16,185,129,0.05)]" : "border-[#FAF7F2]"
                }`}
        >
            {/* Top Header */}
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        disabled={isApproved}
                        className={`px-3 py-1.5 rounded-xl text-xs font-extrabold transition-all cursor-pointer ${isApproved
            ...
            {isEditing ? (
                <div className="flex flex-col gap-4 pt-2">
                    <fieldset className="flex flex-wrap items-center gap-4 rounded-xl border border-[#FAF7F2] p-3">
                        ...
                        {draftOptions.map((opt, oIdx) => (
                            <div key={oIdx} className="flex items-center gap-2">
                                ...
                                <input
                                    ...
                                    className={`w-full p-2.5 rounded-xl border text-xs font-medium ${draftCorrectIndices.includes(oIdx) ? "-[#2C3039] bg-emerald-50/50 font-bold -[#2C3039]" : "border-[#FAF7F2]"
                                        }`}
                                />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div className="md:col-span-2">
                            ...
                            <input
                                ...
                                className="w-full p-2.5 rounded-xl border border-[#FAF7F2] text-xs font-medium"
                            />
                        </div>
                        <div>
                            ...
                            <input
                                ...
                                className="w-full p-2.5 rounded-xl border border-[#FAF7F2] text-xs font-bold focus:border-[#C0392B] focus:outline-none"
                            />
                        </div>
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-1">
                        {question.options.map((opt, oIdx) => {
                            ...
                            return (
                                <div
                                    key={oIdx}
                                    className={`p-3 rounded-xl border flex items-center gap-3 text-xs font-semibold ${isCorrect
                                        ? "-[#2C3039] bg-emerald-50 -[#2C3039] font-bold shadow-xs"
                                        : "border-[#FAF7F2] bg-[#FEFCF9] text-gray-700"
                                        }`}
                                >
                                    <span
                                        className={`w-6 h-6 rounded-full flex items-center justify-center font-black text-xs ${isCorrect ? "-[#2C3039] text-white" : "bg-gray-200 text-[#8A8478]"
                                            }`}
                                    >
