...
export function LessonAIQuizModal({ lessonTitle = "Building Type-Safe Server Actions", isOpen, onClose, onConfirmDecks }: LessonAIQuizModalProps) {
 ...
 <span className="text-[10px] font-mono font-extrabold px-2 py-0.5 rounded-md bg-white/10 text-[#3B82F6] border border-white/10">
 Rapid Review UI
 </span>
 </div>
 ...
 <span className="px-3 py-1.5 rounded-xl bg-[#0F172A]/20 text-[#F8FAFC] border-[#0F172A]/30 text-xs font-black">
 ...
 {questions.length === 0 && (
 <div className="p-6 rounded-3xl bg-white border border-[#E2E8F0] shadow-xs flex flex-col gap-4">
 <div className="flex flex-col gap-4 animate-fadeIn">
 <div className="flex items-center gap-3">
 
 <span className="text-sm font-black text-[#0F172A]">Source Material</span>
 </div>
 ...
 {error && (
 <div className="p-3 bg-[#EFF6FF] border border-[#DBEAFE] text-[#2563EB] rounded-xl text-xs font-bold">
 {error}
 </div>
 )}
 <textarea
 ...
 rows={5}
 className="w-full p-4 rounded-2xl border-[#E2E8F0] bg-[#F8F9FF] text-xs font-medium text-gray-700 leading-relaxed focus:outline-none focus:border-[#E2E8F0] transition-colors"
 ...
 className="self-end px-8 py-3 bg-[#3B82F6] text-white text-xs font-black rounded-2xl shadow-lg hover:scale-[1.02] transition-all disabled:opacity-50"
 >
 ...
 <Loader size="md" />
 <h4 className="text-base font-extrabold text-[#0F172A]">Analyzing lesson transcript semantics...</h4>
 ...
 <div>
 <h4 className="text-sm font-extrabold text-[#0F172A]">Rapid-Review Interface</h4>
 <p className="text-xs text-[#64748B] font-semibold">
 You retain full editorial control: Click <strong className="text-[#0F172A]">Approve</strong>, <strong className="text-[#3B82F6]">Edit</strong>, or <strong className="text-[#3B82F6]">Discard</strong>.
 </p>
 </div>
 <button
 type="button"
 ...
 isApproved
 ? "text-[#0F172A]/50 bg-emerald-50/10 shadow-[0_4px_20px_rgba(16,185,129,0.05)]"
 : "border-[#E2E8F0]"
 )}
 >
 {/* Top Tag & Status */}
 ...
 <div className="flex items-center gap-2">
 <span className="w-7 h-7 rounded-xl bg-blue-50 text-[#3B82F6] font-black text-xs flex items-center justify-center">
 #{idx + 1}
 </span>
 <span className="text-[11px] font-black uppercase px-2.5 py-1 rounded-lg bg-gray-100 text-[#64748B] border">
 {q.type.replace("_", " ")}
 </span>
 {isApproved && (
 ...
 isApproved ? "text-[#0F172A] text-white cursor-default" : "bg-emerald-50 hover:bg-[#0F172A] text-[#0F172A] hover:text-white border-[#E2E8F0]"
 )}
 >
 ...
 className="px-3.5 py-1.5 rounded-xl bg-blue-50 hover:bg-[#2563EB] text-[#3B82F6] hover:text-white border-[#E2E8F0] text-xs font-extrabold transition-all"
 >
 ...
 className="px-3.5 py-1.5 rounded-xl bg-[#EFF6FF] hover:bg-[#2563EB] text-[#2563EB] hover:text-white border border-[#DBEAFE] text-xs font-extrabold transition-all"
 >
 Discard
 </button>
 </div>
 ...
 <div>
 <label className="block text-xs font-bold text-[#64748B] uppercase mb-1">Question Text</label>
 <input
 type="text"
 value={draftQ}
 onChange={(e) => setDraftQ(e.target.value)}
 className="w-full p-3 rounded-xl border border-[#E2E8F0] font-bold text-sm text-[#0F172A]"
 />
 </div>
 <div>
 <label className="block text-xs font-bold text-[#0F172A] uppercase mb-1">Correct Answer</label>
 <input
 type="text"
 value={draftA}
 onChange={(e) => setDraftA(e.target.value)}
 className="w-full p-3 rounded-xl border-[#0F172A] font-extrabold text-sm text-[#0F172A] bg-emerald-50/50"
 />
 </div>
 </div>
 ) : (
 <div className="flex flex-col gap-3">
 <h5 className="text-base font-extrabold text-[#0F172A] leading-snug">
 {q.question}
 </h5>

 {q.codeSnippet && (
 <pre className="p-3.5 rounded-2xl bg-[#1E233E] text-[#F8FAFC] font-mono text-xs overflow-x-auto border-[#3B82F6]/30">
 <code>{q.codeSnippet}</code>
 </pre>
 )}

 <div className="flex flex-col gap-2">
 <div className="flex items-center gap-3 p-3 rounded-xl bg-emerald-50 border-[#E2E8F0] font-bold text-sm text-[#0F172A]">
 
 <span>Correct Answer: {q.correctAnswer}</span>
 </div>

 {q.distractors.map((dist, dIdx) => (
 <div key={dIdx} className="flex items-center gap-3 p-2.5 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] text-xs font-medium text-[#64748B]">
 ...
 <div className="p-12 text-center rounded-3xl bg-white border border-[#E2E8F0] flex flex-col items-center gap-3 text-[#64748B]">
 <span className="text-4xl"></span>
 <p className="text-sm font-bold text-[#0F172A]">All generated questions were discarded.</p>
 <button
 type="button"
 onClick={() => generateFromTranscript(lessonTitle)}
 ...
 {/* Modal Footer */}
 <div className="p-4 px-6 bg-white border-t border-[#E2E8F0] flex items-center justify-between">
 ...
 className="px-6 py-2.5 bg-[#0F172A] hover:bg-[#2563EB] text-white text-xs font-black rounded-xl shadow-lg transition-all disabled:opacity-50 cursor-pointer"
 >
