    ...
    <section className="flex flex-col gap-3 rounded-xl border border-[#E8E2D9] bg-[#FEFCF9] p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          {!readOnly && <p className="text-xs text-[#8A8478]">DOC, Excel, PowerPoint hoặc PDF — tối đa 25 MB/file.</p>}
        </div>
        {!readOnly && (
          <label className="cursor-pointer rounded-lg bg-[#C0392B] px-3 py-2 text-xs font-bold text-white">
      ...
      {attachments.length === 0 ? (
        <p className="text-xs text-[#8A8478]">Chưa có tài liệu đính kèm.</p>
      ) : (
        <ul className="flex flex-col gap-2">
          {attachments.map((attachment) => (
            <li key={attachment.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-[#E8E2D9] bg-white p-3">
              ...
              <span className="text-xs uppercase text-[#8A8478]">{attachment.extension} · {formatBytes(attachment.size_bytes)}</span>
              {!readOnly && (
                <button type="button" aria-label={`Lưu tên ${names[attachment.id]}`} onClick={() => void handleRename(attachment)} className="text-xs font-bold text-[#C0392B]">Lưu tên</button>
              )}
              <button type="button" aria-label={`Tải ${names[attachment.id] ?? attachment.display_name}`} onClick={() => void handleDownload(attachment)} className="text-xs font-bold text-[#C0392B]">Tải xuống</button>
              {!readOnly && (
