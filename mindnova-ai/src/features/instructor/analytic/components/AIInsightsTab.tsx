...
export function AIInsightsTab() {
    ...
    return (
        <div className="w-full flex flex-col gap-5 animate-fadeIn">
            {/* Header Summary Box */}
            <div className="p-6 rounded-2xl bg-[#C0392B] text-white border border-white/20 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                <div className="flex items-center gap-4">

                    <div>
                        <div className="flex flex-wrap items-center gap-2">
                            <h3 className="text-base font-black text-white">Đề Xuất Cải Tiến Nội Dung AI Chuyên Sâu</h3>
                            <span className="text-[11px] font-mono font-bold px-2.5 py-0.5 rounded-full bg-[#2C3039]/30 text-white border border-white/40">
                                Live Behavioral Telemetry
                            </span>
                        </div>
                        <p className="text-xs text-[#FAF7F2] max-w-xl mt-1">
                ...
                <div className="flex items-center gap-1 p-1 rounded-xl bg-white/10 border border-white/10 text-xs font-bold shrink-0">
                    <button
                        ...
                        className={twMerge(
                            ...
                            filter === "unresolved" ? "bg-rose-500 text-white font-extrabold shadow-2xs" : "text-white/80 hover:bg-white/10"
                        )}
                    >
                    ...
                    <button
                        ...
                        className={twMerge(
                            "px-3.5 py-1.5 rounded-lg transition-all cursor-pointer",
                            filter === "all" ? "bg-rose-500 text-white font-extrabold shadow-2xs" : "text-white/80 hover:bg-white/10"
                        )}
                    >
            ...
            {/* Insights List Grid */}
            {filtered.length === 0 ? (
                <div className="p-14 text-center rounded-2xl bg-white border border-[#FAF7F2] shadow-2xs flex flex-col items-center gap-2 text-[#8A8478]">
                        ...
                        <div
                            ...
                            className={twMerge(
                                ...
                                item.priority === "high" && !item.isResolved
                                    ? "border-[#FAF7F2] bg-rose-50/10"
                                    : "border-[#FAF7F2]",
                                item.isResolved && "opacity-60 bg-[#FEFCF9]/50 border-[#FAF7F2]"
                            )}
                        >
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                    <span
                                        className={twMerge(
                                            ...
                                            item.type === "suggestion" && "bg-emerald-50 text-[#2C3039] border-[#FAF7F2]",
                                            item.type === "trend" && "bg-purple-50 text-[#C0392B] border-[#FAF7F2]"
                                        )}
                                    >
                                ...
                                {item.metrics && Object.keys(item.metrics).length > 0 && (
                                    <div className="flex items-center gap-3 pl-1 mt-1">
                                        {Object.entries(item.metrics).map(([k, v]) => (
                                            <div key={k} className="text-xs bg-gray-100 rounded px-2 py-1 border border-[#FAF7F2] text-gray-700">
                            ...
                            <div className="flex sm:flex-col gap-2 shrink-0 w-full md:w-auto justify-end">
                                ...
                                {!item.isResolved && (
                                    <button
                                        ...
                                        className="flex-1 md:flex-none px-4 py-2 rounded-xl bg-gray-100 hover:bg-emerald-50 text-gray-700 hover:text-[#2C3039] font-bold text-xs transition-all border border-[#FAF7F2] cursor-pointer"
                                    >
