  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center p-20 gap-4">
        <Loader size="md" />
        <p className="text-sm font-bold text-gray-500">Đang phân tích dữ liệu học viên bằng AI...</p>
      </div>
    );
  }
          ...
          {filtered.map((item) => (
            ...
            >
              <div className="flex flex-col gap-2.5 flex-1">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span
                    className={twMerge(
                      ...
                      item.type === "suggestion" && "bg-emerald-50 text-emerald-800 border-emerald-200",
                      item.type === "trend" && "bg-purple-50 text-purple-800 border-purple-200"
                    )}
                  >
                ...
                {item.metrics && Object.keys(item.metrics).length > 0 && (
                  <div className="flex items-center gap-3 pl-1 mt-1">
                    {Object.entries(item.metrics).map(([k, v]) => (
                      <div key={k} className="text-xs bg-gray-100 rounded px-2 py-1 border border-gray-200 text-gray-700">
                ...
                <div className="p-3.5 mt-2 rounded-xl bg-indigo-50/60 border border-indigo-100 text-xs font-medium text-indigo-950 leading-relaxed flex flex-col gap-1.5">
                  <div className="flex items-center gap-2">
                    <span className="text-base shrink-0">💡</span>
                    <strong className="font-extrabold text-[#4F46E5]">Đề xuất Kế hoạch hành động: </strong>
                  </div>
