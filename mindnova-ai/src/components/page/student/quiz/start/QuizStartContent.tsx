...
export function QuizStartContent() {
  ...
  const isAttemptsExhausted = attemptsLeft !== null && attemptsLeft <= 0;

  // ── Business Logic: Deadline ───────────────────────────────────────────────
  const isDeadlinePassed = quiz.deadline !== null
    ? new Date(quiz.deadline) < new Date()
    : false;

  const isBlocked = isAttemptsExhausted || isDeadlinePassed;
  ...
  const attemptsDisplay = quiz.maxAttempts === null
    ? "Unlimited"
  ...
  const handleStart = useCallback(() => {
    ...
    // Store quiz session startedAt on the server side.
    // In production: POST /api/quizzes/[quizId]/sessions → returns { sessionId, startedAt }
    // Here we simulate by writing startedAt to sessionStorage.
    const startedAt = Date.now();
    sessionStorage.setItem("quizSession", JSON.stringify({ quizId: quiz.id, startedAt }));

    router.push("/practice/quiz/question");
  }, [isBlocked, quiz.id, router]);

  // ─── Blocked State ────────────────────────────────────────────────────────
  if (isBlocked) {
    return (
      <div className="flex-1 overflow-y-auto bg-white relative min-h-full flex items-center justify-center p-8">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-2xl font-bold text-[#1F2937] mb-3">
            {isDeadlinePassed ? "Deadline Has Passed" : "No Attempts Remaining"}
          </h2>
          <p className="text-gray-500 leading-relaxed mb-6">
            {isDeadlinePassed
              ? `The submission deadline for "${quiz.title}" has passed. No new attempts are allowed.`
            ...
            <div className="bg-white rounded-3xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#F3F4F6]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-8 gap-x-6">
                <StatItem
                  ...
                  label="Questions"
                  value={`${quiz.questions.length} Items`}
                  iconBg="bg-[#EEF2FF]"
                ...
                <StatItem
                  ...
                  label="Time Limit"
                  value={formatDuration(quiz.durationSeconds)}
                  iconBg="bg-[#E0F2FE]"
                ...
                <StatItem
                  ...
                  label="Passing Score"
                  value={`${quiz.passingScore}% (${Math.ceil(quiz.questions.length * quiz.passingScore / 100)}/${quiz.questions.length})`}
                  iconBg="bg-[#F3E8FF]"
                  iconColor="text-[#9333EA]"
                />
                {/* Attempts — with color coding */}
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className={`text-[17px] font-bold ${
                      attemptsLeft !== null && attemptsLeft <= 1 ? "text-[#EF4444]" : "text-gray-900"
                    }`}>
                      {attemptsDisplay}
                    </p>
                    {attemptsLeft !== null && attemptsLeft <= 1 && (
                      <p className="text-[11px] text-[#EF4444] font-semibold mt-0.5">⚠ Last attempt!</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <ul className="space-y-4">
                  {[
                    "The timer starts on the server the moment you click Start Quiz — refreshing the page will NOT reset the timer.",
                    "Answers are saved automatically. The quiz will auto-submit when the timer expires.",
                    "Once all attempts are used, you cannot retake the quiz.",
                  ].map((text, i) => (
            ...
            <div className="bg-gradient-to-r from-white to-[#EEF2FF]/60 border border-[#E0E7FF] rounded-2xl p-6 flex gap-5 items-start shadow-sm">
              ...
              <div className="pt-0.5">
                ...
                <p className="text-[#4B5563] leading-relaxed text-[15px]">
                  Most students find question 4 and 7 challenging in this module. Focus on the
                  differences between Request and Response objects in standard Web APIs versus
                  Next.js wrappers.
                </p>
              </div>
            </div>
            ...
            <div className="border border-dashed border-[#D1D5DB] rounded-[20px] p-5 bg-transparent">
              ...
              {quiz.attemptsUsed > 0 ? (
                <p className="text-[13px] text-gray-500 leading-relaxed pl-6">
                  {quiz.attemptsUsed} previous attempt{quiz.attemptsUsed !== 1 ? "s" : ""}.
                  <br />
                  {attemptsLeft !== null
