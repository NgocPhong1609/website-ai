...
function Topbar() {
  return (
    <header className="h-16 shrink-0 flex items-center gap-4 px-8 bg-white border-b border-[#F0F0F8]">
      <h1 className="text-[15px] font-extrabold text-[#1A1A2E] tracking-tight">
      ...
      <div className="flex items-center gap-1.5 text-[#7878A0]">
        <button
          type="button"
          className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-[#F4F4FA] hover:text-[#4648D4] transition-all relative"
        >
          <BellIcon size={17} />
        </button>
        <button
          type="button"
          className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-[#F4F4FA] hover:text-[#4648D4] transition-all"
        >
          <MessageIcon size={17} />
        </button>
        <button
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
      {stats.map((s, i) => (
        <div
          key={i}
          className="bg-white rounded-2xl p-5 border border-[#EAEAF4] shadow-sm flex flex-col relative overflow-hidden"
        >
          <span className="text-[12px] font-semibold text-[#64647A] tracking-wide">
            {s.label}
          </span>
          <div className="flex items-end gap-2 mt-2">
            <span className="text-[20px] font-extrabold text-[#1A1A2E] leading-tight">
              {s.val}
            </span>
            <span
              className={twMerge(
                "text-[10px] font-bold mb-1",
                s.isUp ? "text-emerald-500" : "text-rose-500",
              )}
            >
              {s.diff}
            </span>
          ...
          <div className="w-full h-1 bg-[#F0F0F8] rounded-full mt-4 overflow-hidden">
            <div
              className={twMerge("h-full rounded-full", s.color)}
              style={{ width: `${s.width}%` }}
            />
          </div>
        </div>
    ...
    <div className="bg-white rounded-2xl border border-[#EAEAF4] shadow-sm p-6 mb-6 flex flex-col h-[360px]">
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-[12px] text-[#9090B0] mt-1">
            Daily performance tracking for October 2023
          </p>
        </div>
        <div className="flex items-center gap-5">
          <div className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-full bg-[#4648D4]" />
            <span className="text-[12px] font-semibold text-[#64647A]">
              Revenue
            </span>
          </div>
          <div className="flex items-center gap-2">
      ...
      <div className="flex-1 relative mt-2 w-full">
        ...
        <div className="absolute inset-0 flex flex-col justify-between border-b border-[#F0F0F8]">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="w-full h-px bg-[#F0F0F8]" />
          ))}
        </div>
        ...
        >
          <path
            d="M0,150 L100,130 L200,140 L300,100 L400,80 L500,110 L600,60 L700,50 L800,70 L900,40 L1000,50"
            fill="none"
            stroke="#4648D4"
            strokeWidth="2.5"
          />
          <path
            d="M0,190 L100,188 L200,189 L300,185 L400,186 L500,184 L600,185 L700,186 L800,182 L900,180 L1000,183"
            fill="none"
            stroke="#F83A3A"
            strokeWidth="1.5"
            strokeDasharray="4 4"
        ...
        <div className="absolute left-[70%] top-[10%] -translate-x-1/2 -translate-y-full mb-2 bg-white rounded-lg shadow-[0_4px_20px_rgba(0,0,0,0.08)] border border-[#EAEAF4] p-3 w-max z-10 pointer-events-none">
          <span className="block text-[9px] font-bold text-[#9090B0] uppercase tracking-widest mb-1">
            OCT 22, 2023
          </span>
          <div className="flex items-center gap-3">
            <span className="text-[14px] font-extrabold text-[#4648D4]">
              $2,840.00
            </span>
            <span className="text-[14px] font-extrabold text-[#F83A3A]">
              $42.00
            </span>
          </div>
        </div>
    ...
    <div className="bg-white rounded-2xl border border-[#EAEAF4] shadow-sm flex flex-col overflow-hidden mb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-[#F0F0F8] px-2 pt-2">
        <div className="flex items-center gap-1">
          <button className="px-5 py-3 text-[13px] font-bold text-[#4648D4] border-b-2 border-[#4648D4] bg-[#F8F8FD]">
            Marketing Sources
          </button>
          <button className="px-5 py-3 text-[13px] font-semibold text-[#64647A] hover:text-[#1A1A2E] hover:bg-[#FAFAFE] rounded-t-lg transition-colors border-b-2 border-transparent">
            Demographics
          </button>
          <button className="px-5 py-3 text-[13px] font-semibold text-[#64647A] hover:text-[#1A1A2E] hover:bg-[#FAFAFE] rounded-t-lg transition-colors border-b-2 border-transparent">
            Course Categories
        ...
        <div className="flex items-center gap-4 px-4 py-2 sm:py-0">
          <span className="text-[12px] font-medium text-[#9090B0]">
            Showing 1–10 of 24 sources
          </span>
          <div className="flex items-center gap-1.5">
            <button className="w-7 h-7 flex items-center justify-center rounded border border-[#EAEAF4] text-[#B0B0C8] hover:text-[#464554] hover:bg-[#F4F4FA]">
              <ChevronLeftIcon />
            </button>
            <button className="w-7 h-7 flex items-center justify-center rounded border border-[#EAEAF4] text-[#464554] hover:bg-[#F4F4FA]">
              <ChevronRightIcon />
            </button>
          </div>
        </div>
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className="border-b border-[#F0F0F8]">
              <th className="px-6 py-4 text-[11px] font-bold text-[#9090B0] uppercase tracking-wide">
                Source Channel
              </th>
              <th className="px-6 py-4 text-[11px] font-bold text-[#9090B0] uppercase tracking-wide">
                Total Leads
          ...
          <tbody>
            {sources.map((s, i) => (
              <tr
                key={i}
                className="border-b border-[#F0F0F8] last:border-0 hover:bg-[#FAFAFE] transition-colors"
              >
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] font-medium text-[#9090B0]">
                        {s.sub}
                      </span>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4 text-[13px] font-semibold text-[#464554]">
                  {s.leads}
                </td>
                <td className="px-6 py-4 text-[13px] font-semibold text-[#464554]">
                  {s.conv}
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    <span className="text-[13px] font-extrabold text-[#464554] w-[45px]">
                      {s.rate}%
                    </span>
                    <div className="flex-1 h-1.5 rounded-full bg-[#F0F0F8] overflow-hidden max-w-[60px]">
                      <div
                        className="h-full rounded-full bg-[#A3A5F8]"
                        style={{ width: `${s.rate * 3}%` }}
                      />
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4 text-[13px] font-extrabold text-[#1A1A2E]">
                  {s.rev}
                </td>
                <td className="px-6 py-4 text-center">
                  {s.trend === "up" ? (
                    ...
                    <span className="text-[#B0B0C8] inline-block">
      ...
      <div className="p-4 flex items-center justify-center border-t border-[#F0F0F8] bg-[#FAFAFE]">
        <button className="text-[12px] font-bold text-[#6B6BFF] hover:underline">
          View All Channel Performance
        </button>
      </div>
    </div>
  );
        ...
        <div className="max-w-[1200px] mx-auto px-6 lg:px-8 py-8">
          ...
          <footer className="flex flex-col md:flex-row items-center justify-between gap-4 py-6 border-t border-[#EAEAF4] mt-8">
            <span className="text-[11px] font-medium text-[#9090B0]">
              © 2023 EduMaster Pro Instructor Console. All sales data is
              real-time.
            </span>
            <div className="flex items-center gap-4 text-[11px] font-medium text-[#9090B0]">
              <a href="#" className="hover:text-[#464554] transition-colors">
                Privacy Policy
              </a>
              <a href="#" className="hover:text-[#464554] transition-colors">
