...
test('student can update their own course review', function () {
    $teacher = User::factory()->create(['role' => 'teacher', 'email_verified_at' => now()]);
    $student = User::factory()->create(['role' => 'student', 'email_verified_at' => now()]);
    ...
    $course = Course::create([
        'teacher_id' => $teacher->id,
        'category_id' => null,
        'title' => 'AI Sample Course 3',
        'slug' => 'ai-sample-course-reviews-3',
        'description' => 'Course for review update',
        'price' => 0,
        'level' => 'beginner',
        'status' => 'published',
        'published_version_id' => 1,
    ...
    $review = Review::create([
        ...
        'user_id' => $student->id,
        'rating' => 4,
        'comment' => 'Bài giảng khá tốt',
    ]);

    $response = $this->actingAs($student, 'sanctum')->putJson("/api/student/courses/{$course->id}/reviews/{$review->id}", [
        'rating' => 5,
        'comment' => 'Bài giảng rất tốt sau khi cập nhật',
    ]);

    $response->assertOk()
        ->assertJsonPath('data.comment', 'Bài giảng rất tốt sau khi cập nhật')
        ->assertJsonPath('data.rating', 5);
...
test('student can delete their own course review', function () {
    $teacher = User::factory()->create(['role' => 'teacher', 'email_verified_at' => now()]);
    $student = User::factory()->create(['role' => 'student', 'email_verified_at' => now()]);
    ...
    $course = Course::create([
        'teacher_id' => $teacher->id,
        'category_id' => null,
        'title' => 'AI Sample Course 4',
        'slug' => 'ai-sample-course-reviews-4',
        'description' => 'Course for review delete',
        'price' => 0,
        'level' => 'beginner',
        'status' => 'published',
        'published_version_id' => 1,
    ...
    $review = Review::create([
        ...
        'user_id' => $student->id,
        'rating' => 3,
        'comment' => 'Nên cải thiện thêm',
    ]);

    $response = $this->actingAs($student, 'sanctum')->deleteJson("/api/student/courses/{$course->id}/reviews/{$review->id}");

    $response->assertOk()
        ->assertJsonPath('message', 'Xoá đánh giá thành công.');
